<template>
  <div>
    <vui-input-search :threshold="0.4" :disabled="$root.$data.status < 2" :input="state.smeslist" v-model="filteredSMES" :keys="['RCON_tag']"/>
      ...
      <vui-group>
        <div v-for="smes in state.smeslist" :key="smes.uuid">
          <div v-show="filteredSMES.indexOf(smes)>=0">
            ...
            <span style="padding: 0; display: inline-block; line-height: 0; height: 4px; margin: 0;"/>
          </div>
        </div>
      </vui-group>
    </span>
